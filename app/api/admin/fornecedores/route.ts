...
const DEFAULT_ORDER = `CASE WHEN mc.verified_cnpj AND mc.has_confirmed_address AND mc.has_confirmed_shipping THEN 0
     WHEN mc.verified_cnpj THEN 1 ELSE 2 END ASC,
     mc.avg_response_time_minutes ASC NULLS LAST, mc.short_name ASC`
...
export async function GET(req: NextRequest) {
  ...
  const rows = await db.query(
    ...
    WHERE mc.has_confirmed_configuration = true
      ...
      AND ($3 = 'all'
           OR ($3 = 'certified' AND mc.verified_cnpj AND mc.has_confirmed_address AND mc.has_confirmed_shipping)
           OR ($3 = 'validated' AND mc.verified_cnpj AND NOT (mc.has_confirmed_address AND mc.has_confirmed_shipping))
           OR ($3 = 'basic'     AND NOT mc.verified_cnpj))
