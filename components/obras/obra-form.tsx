...
const EMPTY: FormState = {
  name: "", status: "Orçamento", type: "", area_m2: "",
  start_date: "", expected_end_date: "",
  is_own: false, client_id: "",
  delivery_zipcode: "", delivery_street: "", delivery_number: "",
  delivery_complement: "", delivery_neighbourhood: "", delivery_city: "", delivery_state: "",
...
interface ObraFormProps {
  /** "create" para nova obra, "edit" para edição de obra existente */
  mode: "create" | "edit"
  /** Obrigatório no modo "edit": id da obra a ser editada */
  obraId?: string
}

/**
 * Formulário compartilhado de obra usado tanto na criação quanto na edição.
 * - mode="create": faz POST /api/obras
 * - mode="edit":   carrega a obra via GET e faz PUT /api/obras/[id]
 */
export function ObraForm({ mode, obraId }: ObraFormProps) {
  ...
  // Carrega dados da obra (apenas no modo edição)
  useEffect(() => {
  ...
  // Carrega lista de clientes da empresa ativa
  useEffect(() => {
  ...
  const filteredClients = clientSearch.trim()
    ? clients.filter((c) => {
        ...
        return name.includes(normalize(clientSearch))
      })
    : clients.slice(0, 8)
  ...
  async function fetchCep(raw: string, type: "delivery" | "billing") {
    ...
    try {
      ...
      if (type === "delivery") {
        syncBilling({
          delivery_zipcode: formatted,
          delivery_street: json.logradouro ?? "",
          delivery_neighbourhood: json.bairro ?? "",
          delivery_city: json.localidade ?? "",
          delivery_state: json.uf ?? "",
          delivery_complement: json.complemento ?? "",
        })
      } else {
        setForm((p) => ({
          ...p,
          billing_street: json.logradouro ?? "",
          billing_neighbourhood: json.bairro ?? "",
          billing_city: json.localidade ?? "",
          billing_state: json.uf ?? "",
        }))
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!validate()) return
    if (!isEdit && !activeCompany?.id) { toast.error("Selecione uma empresa"); return }
    setLoading(true)

    // Payload compartilhado entre criação e edição
    const payload: Record<string, unknown> = {
      ...
      delivery_zipcode: form.delivery_zipcode || null,
      ...
      delivery_neighbourhood: form.delivery_neighbourhood || null,
      delivery_city: form.delivery_city || null,
      delivery_state: form.delivery_state || null,
      same_billing_address: form.same_billing_address,
    ...
    try {
      const res = isEdit
        ? await authFetch(`/api/obras/${obraId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          })
        : await authFetch("/api/obras", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ...payload, company_id: activeCompany!.id }),
          })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? "Erro ao salvar")
      toast.success(isEdit ? "Obra atualizada com sucesso!" : "Obra cadastrada com sucesso!")
      router.push(isEdit ? `/dashboard/obras/${obraId}` : "/dashboard/obras")
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <form id="obra-form" onSubmit={handleSubmit} noValidate className="flex-1 overflow-y-auto" style={{ padding: "0 16px" }}>
            ...
            {selectedClient ? (
              <div className="flex items-center justify-between bg-[#F5F7FF] rounded-lg px-3 py-2.5 border border-[#1565C0]">
                <div>
                  <p className="text-sm font-medium text-[#212121]">
                    {selectedClient.full_name ?? selectedClient.fantasy_name ?? selectedClient.company_name}
                  </p>
              ...
              <div className="relative">
                <div className="flex items-center gap-2 border border-[#E0E0E0] rounded-lg px-3 py-2.5 focus-within:border-[#1565C0] transition-colors">
                  ...
                  <input
                    className="flex-1 outline-none text-sm text-[#212121] placeholder:text-[#9E9E9E] bg-transparent"
        ...
        <OpInput
          ...
          onChange={(e) => fetchCep(e.target.value, "delivery")}
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1"><OpInput label="Complemento" value={form.delivery_complement} onChange={(e) => syncBilling({ delivery_complement: e.target.value })} placeholder="Apto, Sala..." /></div>
        </div>
        ...
        <label className="flex items-center gap-2 py-2 cursor-pointer mb-3">
          <input
            ...
            onChange={(e) => {
              ...
              if (v) {
                setForm((p) => ({
                  ...p, same_billing_address: true,
                  billing_zipcode: p.delivery_zipcode,
                  billing_street: p.delivery_street,
                  billing_number: p.delivery_number,
                  billing_complement: p.delivery_complement,
                  billing_neighbourhood: p.delivery_neighbourhood,
                  billing_city: p.delivery_city,
                  billing_state: p.delivery_state,
                }))
              }
            }}
        ...
        {!form.same_billing_address && (
          <>
            <OpInput
              ...
              onChange={(e) => fetchCep(e.target.value, "billing")}
              placeholder="00000-000"
            />
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1"><OpInput label="Complemento" value={form.billing_complement} onChange={(e) => set("billing_complement", e.target.value)} placeholder="Apto, Sala..." /></div>
            </div>
